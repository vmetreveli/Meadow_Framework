...
namespace Meadow.Framework.Mediator.Infrastructure.Events;
    ...
    IUnitOfWork unitOfWork) : IEventDispatcher
{
    ...
    //         await repository.UpdateOutboxMessageState(@event.Id, IntegrationEventStatus.Published);
    //     }
    //     catch (Exception ex)
    //     {
    //         // In case of failure, store the event in the outbox for retrying.
    //         var outboxMessage = new Meadow.Framework.IntegrationEvents.OutboxMessage(@event, @event.Id, @event.CreationDate);
