...
namespace Meadow.Framework.Infrastructure.Infrastructure.Interceptors;
...
public sealed class UpdateDeletableEntitiesInterceptor : SaveChangesInterceptor
{
    ...
    /// <returns>
    ///     A <see cref="ValueTask{InterceptionResult}" /> representing the asynchronous operation, with the original or
    ///     modified result.
    /// </returns>
    /// <remarks>
    ...
    {
        if (eventData.Context is not null) UpdateDeletableEntities(eventData.Context);
    ...
    /// <remarks>
    ///     This method filters for entities that implement <see cref="IDeletableEntity" /> and are in the
    ///     <see cref="EntityState.Deleted" /> state.
    ///     It then changes their state to <see cref="EntityState.Modified" /> and sets the
    ///     <see cref="IDeletableEntity.DeletedOn" />
